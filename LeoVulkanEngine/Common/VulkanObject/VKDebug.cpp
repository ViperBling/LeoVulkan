...
namespace LeoVK::Debug
{
        ...
        void* pUserData)
    {
        ...
        // 将结果输出到Console或者Logcat中
        std::stringstream debugMsg;
        debugMsg << prefix << "[" <<pCallbackData->messageIdNumber << "][" << pCallbackData->pMessageIdName << "] : " << pCallbackData->pMessage;

        if (messageSeverity >= VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT) {
            std::cerr << debugMsg.str() << "\n";
        } else {
...
namespace LeoVK::DebugMarker
{
    ...
    void Setup(VkDevice device)
    {
        ...
        bActive = (pfnDebugMarkerSetObjectName != VK_NULL_HANDLE);
    }
    ...
    void SetDescriptorSetLayoutName(
        VkDevice device,
